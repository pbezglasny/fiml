...
flowchart LR
    ...
    Entry *-- Adapter["IndicatorAdapter<F>"]
```
...
| `CompiledFeature<F>` | Internal | Owns one `IndicatorAdapter<F>` plus its `FeatureRoute`. |
...
`IndicatorAdapter` enum.

| `IndicatorSpec` variant | `IndicatorAdapter` variant | Concrete internal adapter | Standalone calculation state | Route |
|---|---|---|---|---|
...
The concrete adapter structs and `IndicatorAdapter` are internal. Downstream
...
flowchart LR
    ...
    IFV *--|M slots| Adapters["IndicatorAdapter<F>"]
...
|---|---|---|
| `IndicatorFeatureVector<F, V, M>` | Public | Owns fixed-capacity `IndicatorAdapter<F>` storage, caller-provided `V: FeatureVector`, route ranges, timed-adapter indexes, names, and timestamp watermark. |
