...
- `FeatureExtractorSpec` describes one output cell per `FeatureDef`.
...
```text
FeatureExtractorSpec or fluent builder
        |
        v
...
```rust
IndicatorFeatureVector::<F, V, M>::from_feature_extractor_spec(cells, &feature_extractor_spec)
```

The runtime-sized `FeatureExtractor` selects a fixed capacity and delegates to
the same interface. The fluent builder produces a reusable `FeatureExtractorSpec`; it
...
`FeatureExtractorSpec` is an ordered collection of `ScopedIndicator` values.
...
than cloning them on every call. Uncompiled `FeatureExtractorSpec` values do not expose
...
The compiler is the single semantic validation path for `FeatureExtractorSpec`
...
6. Replace the parallel fluent builder implementation with a `FeatureExtractorSpec`
   builder.
