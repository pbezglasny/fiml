...
- The public API describes the customization that the extractor actually
  supports: configuring library indicators through `FeatureExtractorSpec` and
  `IndicatorSpec`.
...
- Users can compose standalone calculations in their own applications but
  cannot inject them into `FeatureExtractorSpec`, compilation, or library pipelines as a
